...
# try:
#     # PL >= 1.9: callbacks.base.Callback removed
...
# PL 1.5+: Callback 从 callbacks.base 移到了顶层
try:
    from pytorch_lightning import Callback
except ImportError:
    from pytorch_lightning.callbacks.base import Callback
...
class LogManager(Callback):
    ...
    def on_fit_start(self, trainer, pl_module):
        # PL 1.5+: on_init_end 已移除，改用 on_fit_start
        ckpt_dir = os.path.join(trainer.logger.log_dir, 'checkpoints')
        print("*"*30)
        print('checkpoint_dir', ckpt_dir)
        print("*"*30)
    ...
    def on_exception(self, trainer, pl_module, exception):
        # PL 1.5+: on_keyboard_interrupt 已移除，改用 on_exception
        if not isinstance(exception, KeyboardInterrupt):
            return  # 只处理 KeyboardInterrupt
        if pl_module.global_rank == 0:
            yes = ['yes', 'y']
            no = ['no', 'n']
            ans = ''
