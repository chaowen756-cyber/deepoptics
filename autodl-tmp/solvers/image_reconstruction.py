...
# solvers/image_reconstruction.py (现代化API最终版)
...
def tikhonov_inverse_fast(Y_fft, G_fft, v=None, beta=0, gamma=1e-1):
    """
    使用现代torch.fft API的快速Tikhonov逆。
    输入Y_fft和G_fft应为torch.fft.rfft2的输出。
    """
    device = Y_fft.device
    dtype = Y_fft.dtype  # 应该是复数类型

    # ... 省略了复杂的维度处理，直接使用核心逻辑
    # 假设输入维度是 (B, C, S, D, H, W_half)

    Gc_fft = complex.conj(G_fft)

    # G^H * Y
    GcY = (Gc_fft * Y_fft.unsqueeze(3)).sum(dim=2, keepdim=False)  # B, C, D, H, W_half

    if v is not None:
        V_fft = torch.fft.rfft2(v, dim=(-2, -1))
        GcY = GcY + gamma * V_fft

    # |G|^2 + gamma
    denom = complex.abs2(G_fft).sum(dim=2, keepdim=False) + gamma  # B, C, D, H, W_half
...
def apply_tikhonov_inverse(captimg, psf, reg_tikhonov, apply_edgetaper=True):
    """
    使用现代torch.fft API的Tikhonov逆应用函数。
    """
    if apply_edgetaper:
        captimg = edgetaper3d(captimg, psf)
    ...
    Fcaptimgs = torch.fft.rfft2(captimg, dim=(-2, -1))

    # Fpsf: 1, C, D, H, W_half
    # Fcaptimgs: B, C, H, W_half

    # 增加'shot'维度以匹配tikhonov_inverse_fast的期望输入
    Fpsf = Fpsf.unsqueeze(2)
    Fcaptimgs = Fcaptimgs.unsqueeze(2)

    est_X_fft = tikhonov_inverse_fast(Fcaptimgs, Fpsf, v=None, beta=0, gamma=reg_tikhonov)

    est_volumes = torch.fft.irfft2(est_X_fft, s=captimg.shape[-2:], dim=(-2, -1))
    return est_volumes


# 保留旧函数签名以防测试脚本或其他地方调用，但内部可以简化或直接报错
def tikhonov_inverse(y, g, beta, eps=1e-2):
    raise NotImplementedError("旧的tikhonov_inverse函数未被迁移到新的API，请使用apply_tikhonov_inverse")


def compute_regularized_normal_inv(Hk, beta):
    raise NotImplementedError("旧的compute_regularized_normal_inv函数未被迁移到新的API")